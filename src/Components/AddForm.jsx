...
class AddForm extends React.Component {
  ...
  handleChangeName(event) {
    ...
    if (productName.length) this.setState({ displayLabelName: "visible" });
    else this.setState({ displayLabelName: "hidden" });
  }

  handleChangeQuantity(event) {
    const quantity = event.target.value;
    this.setState({ quantity, displayCheckInputQuantity: "block" });
    if (quantity !== "") this.setState({ displayLabelQuantity: "visible" });
    else this.setState({ displayLabelQuantity: "hidden" });
  }
      ...
      <div className="AddForm">
        <form onSubmit={(event) => this.handleFormSubmit(event)}>
          <div className="row">
            <label
              htmlFor="product"
              style={{ visibility: this.state.displayLabelName }}
            >
              Produs:
            </label>
            <div className="container-input">
              ...
              />
              <Check
                style={{ display: this.state.displayCheckInputName }}
          ...
          <div className="row">
            <label
              htmlFor="quantity"
              style={{ visibility: this.state.displayLabelQuantity }}
            >
              Cantitate:
            </label>
            <div className="container-input">
              <input
                ...
                min="1"
                name="quantity"
                onChange={(event) => this.handleChangeQuantity(event)}
                placeholder="Cantitate"
                value={this.state.quantity}
              />
              <Check
                style={{ display: this.state.displayCheckInputQuantity }}
                className="input-check-icon"
              />
            </div>
          </div>
          ...
          <div className="row">
            ...
            <div className="container-input">
              <select
                name="unit"
                onChange={(event) => this.handleChangeUnit(event)}
              >
